...
struct CircleButton: View {
    ...
    var body: some View {
        ...
        .clipShape(Circle())
        .shadow(color: Color(press ? #colorLiteral(red: 0.7608050108, green: 0.8164883852, blue: 0.9259157777, alpha: 1) : #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)), radius: 20, x: -20, y: -20)
        // 叠加的阴影，是从上层所有阴影的区域延伸出来的，在最底层（设置radius为0就可知）
        .shadow(color: Color(press ? #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1) : #colorLiteral(red: 0.7608050108, green: 0.8164883852, blue: 0.9259157777, alpha: 1)), radius: 20, x: 20, y: 20)
        .scaleEffect(tap ? 1.2 : 1)
        .gesture(
            LongPressGesture()
...
struct PayButton: View {
    ...
    var body: some View {
        ZStack {
            ...
            Image("fingerprint-2")
                .clipShape(
                    // Rectangle 矩形Shape，对这个Shape的y轴进行偏移，控制可显示区域
                    Rectangle()
                        .offset(y: tap ? 0 : 50.0)
                )
                .animation(.easeInOut)
        ...
        .gesture(
            LongPressGesture()
                // .onChanged 是一步到位的变化，而 .updating 可以根据手指触碰多久从而变化多少
                // 也就是说，可以使用tap设置变化前后的值，SwiftUI会自动根据触碰完成进度的百分比去刷新UI
                .updating($tap) { currentState, gestureState, transaction in
                    // currentState：手势的更新状态
                    // gestureState：手势的前一个状态
                    // transaction：手势的上下文
                    
                    // 更新状态（要在这里赋值，否则就没有触碰过程的变化）
